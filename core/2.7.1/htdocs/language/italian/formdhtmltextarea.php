<?php
/**
 * Language definitions for {@link XoopsFormDhtmlTextArea}
 *
 * You may not change or alter any portion of this comment or credits
 * of supporting developers from this source code or any supporting source code
 * which is considered copyrighted (c) material of the original comment or credit authors.
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 *
 * @copyright       (c) 2000-2026 XOOPS Project (https://xoops.org)
 * @license             GNU GPL 2 (https://www.gnu.org/licenses/gpl-2.0.html)
 * @package             kernel
 * @subpackage          form
 * @since               2.3.0
 * @author              Taiwen Jiang <phppp@users.sourceforge.net>
 */
defined('XOOPS_ROOT_PATH') || exit('Restricted access');

// _LANGCODE: it
// _CHARSET : UTF-8
// Translator: XOOPS Translation Team
/**
 * Localized fonts
 */
$GLOBALS['formtextdhtml_fonts'] = [
    'Arial',
    'Courier',
    'Georgia',
    'Helvetica',
    'Impact',
    'Verdana',
    'Haettenschweiler',
];
/**
 * Localized font sizes: 'font size value' => 'font size name'
 */
$GLOBALS['formtextdhtml_sizes'] = [
    'xx-small' => 'xx-Small',
    'x-small'  => 'x-Small',
    'small'    => 'Small',
    'medium'   => 'Medium',
    'large'    => 'Large',
    'x-large'  => 'x-Large',
    'xx-large' => 'xx-Large',
];
define('_XOOPS_FORM_ALT_Inserisci Link', 'URL');
define('_XOOPS_FORM_ALT_EMAIL', 'Indirizzo email');
define('_XOOPS_FORM_ALT_IMG', 'Immagini');
define('_XOOPS_FORM_ALT_IMAGE', 'Immagini interne');
define('_XOOPS_FORM_ALT_SMILEY', 'Faccine');
define('_XOOPS_FORM_ALT_CODE', 'Codice sorgente');
define('_XOOPS_FORM_ALT_QUOTE', 'Citazione');
define('_XOOPS_FORM_ALT_BOLD', 'Grassetto');
define('_XOOPS_FORM_ALT_ITALIC', 'Corsivo');
define('_XOOPS_FORM_ALT_UNDERLINE', 'Sottolineato');
define('_XOOPS_FORM_ALT_LINETHROUGH', 'Barrato');
define('_XOOPS_FORM_ALT_ENTERHEIGHT', 'Altezza:');
define('_XOOPS_FORM_ALT_ENTERWIDTH', 'Larghezza:');
define('_XOOPS_FORM_ALT_LEFT', 'Sinistra');
define('_XOOPS_FORM_ALT_RIGHT', 'Destra');
define('_XOOPS_FORM_ALT_CENTER', 'Centro');
define('_XOOPS_FORM_ALTFLASH', 'Flash');
define('_XOOPS_FORM_ALTMMS', 'MMS');
define('_XOOPS_FORM_ALTRTSP', 'Giocatore reale');
define('_XOOPS_FORM_ALTIFRAME', 'IFRAME');
define('_XOOPS_FORM_ALTWIKI', 'Link WIKI');
define('_XOOPS_FORM_ENTERIFRAMEURL', 'URL IFRAME:');
define('_XOOPS_FORM_ENTERMMSURL', 'URL RMMS:');
define('_XOOPS_FORM_ENTERWMPURL', 'URL WMP:');
define('_XOOPS_FORM_ENTERFLASHURL', 'URL FLASH:');
define('_XOOPS_FORM_ENTERYOUTUBEURL', 'URL Youtube:');
define('_XOOPS_FORM_ENTERRTSPURL', 'URL RTSP:');
define('_XOOPS_FORM_ENTERWIKITERM', 'La parola da collegare al Wiki:');
define('_XOOPS_FORM_ALTMP3', 'MP3');
define('_XOOPS_FORM_ENTERMP3URL', 'URL MP3');
define('_XOOPS_FORM_ALT_CHECKLENGTH', 'Controlla la lunghezza del testo');
define('_XOOPS_FORM_ALT_LENGTH', 'Lunghezza attuale del contenuto: %s');
define('_XOOPS_FORM_ALT_LENGTH_MAX', 'Lunghezza massima: ');
define('_XOOPS_FORM_PREVIEW_CONTENT', 'Clicca su <strong>' . _PREVIEW . '</strong> per vedere il contenuto in azione.');

define('_XOOPS_FORM_ALTYOUTUBE', 'Youtube');
define('_XOOPS_FORM_ENTER_SOUNDCLOUD_URL', 'Inserisci URL SoundCloud');
define('_XOOPS_FORM_ALT_SOUNDCLOUD', 'Soundcloud');
define('_XOOPS_FORM_ALTWMP', 'WMP');

//2.7.1 backfilled translations
define('_XOOPS_FORM_ALT_URL', 'URL');
